...
This checkout has two remotes. Issues and specs live on the **fork**, never on upstream.

| Remote   | Repo               | Role                                                    |
| -------- | ------------------ | ------------------------------------------------------- |
| `origin` | `pingdotgg/t3code` | Upstream. Read-only mirror; local `main` tracks it.     |
| `fork`   | `Khang5687/t3code` | Ours. Issues, fork-only PRs, integration branch `main`. |

`gh repo set-default Khang5687/t3code` is recorded in `.git/config`, which every worktree of this
checkout shares, so bare `gh issue ...` calls resolve to the fork. When in doubt pass
`-R Khang5687/t3code`. Never create issues or non-draft PRs on `pingdotgg/t3code` from a skill.
