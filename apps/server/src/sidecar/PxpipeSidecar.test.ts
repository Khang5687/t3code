  ...
  layer.pipe(
    Layer.provide(
      Layer.mergeAll(
        ...
        ProcessRunner.layer,
        // `Layer.mergeAll` builds in parallel, so the spawner has to be
        // provided under it rather than beside `ProcessRunner.layer`.
      ).pipe(Layer.provideMerge(input.spawner)),
    ),
  );
